...
{
	return alloc_irq_num(req_irq, true);
}
			...
			uint32_t flags)
{
	...
	if (irq == IRQ_INVALID) {
		...
		if (request_irq_arch(irq) == false) {
			pr_err("[%s] failed to request irq %u",
				__func__, irq);
			free_irq_num(irq);
			ret = -EINVAL;
		} else {
...
static void init_irq_descs(void)
{
	...
	for (i = 0U; i < NR_IRQS; i++) {
		desc = &irq_desc_array[i];
		if (irq_allocated_arch(desc)) {
			bitmap_set_nolock((uint16_t)(desc->irq & 0x3FU),
				      irq_alloc_bitmap + (desc->irq >> 6U));
		}
	}
}
